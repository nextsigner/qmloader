...
Unik::Unik(QObject *parent) : QObject(parent)//,
  //m_speech(0)
{
...
void Unik::errorQML(QList<QQmlError> le)
{
    log(le.last().toString().toUtf8());
    setUWarning(le.last().toString());
}
void Unik::ukClose(QQuickCloseEvent *close){
    if(close){
...
void Unik::engineQuited(QObject *)
{
    //db.close();
}

int Unik::getScreenWidth()
{
...
void Unik::setWorkSpace(QString ws)
{
    QSettings settings;
...
void Unik::definirCarpetaTrabajo(QString et)
{
    setWorkSpace(et);
}

bool Unik::folderToUpk(QString folder, QString upkName, QString user, QString key, QString folderDestination)
{
    return mkUpk(folder.toUtf8(), upkName.toUtf8(), user.toUtf8(), key.toUtf8(), folderDestination.toUtf8());
}

bool Unik::carpetaAUpk(QString carpeta, QString nombreUpk, QString usuario, QString clave, QString carpetaDestino)
{
    return folderToUpk(carpeta, nombreUpk, usuario, clave, carpetaDestino);
}

bool Unik::runAppFromZip(QByteArray url, QByteArray localFolder)
{
    QString u;
    u.append(url);
    QString module;
...
bool Unik::downloadRemoteFolder(QString urlFolder, QString list, QString dirDestination)
{
...
bool Unik::mkUpk(QByteArray folder, QByteArray upkName, QByteArray user, QByteArray key, QByteArray folderDestination)
{
    QByteArray dataUpk1="";
    QByteArray sep;
    sep.append(user);
...
bool Unik::upkToFolder(QByteArray upk, QByteArray user, QByteArray key, QByteArray folderDestination)
{
    QFile upkFile(upk);
...
bool Unik::loadUpk(QString upkLocation, bool closeAppLauncher, QString user, QString key)
{
...
void Unik::loadQml(const QString qml)
{
...
void Unik::restartApp(QString args)
{
    qApp->quit();
...
bool Unik::run(QString commandLine){
    return run(commandLine, false, 0);
}

bool Unik::run(QString commandLine, bool waitingForFinished, int milliseconds)
{    
#ifndef Q_OS_ANDROID
...
void Unik::writeRun(QString data)
{
    proc->write(data.toUtf8());
}

bool Unik::ejecutarLineaDeComandoAparte(QString lineaDeComando)
{
#ifndef Q_OS_ANDROID
    proc = new QProcess(this);
    connect(proc, SIGNAL(readyReadStandardOutput()),this, SLOT(salidaRun()));
...
void Unik::salidaRunError()
{
    log(proc->readAllStandardError());
}

void Unik::finalizaRun(int e)
{
    QByteArray s;
...
void Unik::sleep(int ms)
{
    QThread::sleep(ms);
}

QString Unik::getPath(int path)
{
    QString r=".";
...
QString Unik::encData(QByteArray d, QString user, QString key)
{
    QString ret;
    QByteArray upkData;
    QByteArray r="6226";
...
QString Unik::decData(QByteArray d0, QString user, QString key)
{
    QString ret;
...
QQuickWindow *Unik::mainWindow(int n)
{
...
void Unik::setProperty(const QString name, const QVariant &value)
{
    _engine->rootContext()->setProperty(name.toUtf8().constData(), value);
}

QVariant Unik::getProperty(const QString name)
{
    return _engine->rootContext()->property(name.toUtf8());
}

int Unik::getEngineObjectsCount()
{
...
QByteArray Unik::getHttpFile(QByteArray url)
{
...
void Unik::sendFile(QString file, QString phpReceiver)
{
    if(debugLog){
        lba="";
...
void Unik::uploadProgress(qint64 bytesSend, qint64 bytesTotal)
{
...
/*#ifndef Q_OS_ANDROID
void Unik::initWebSocketServer(const QByteArray ip, const int port, const QByteArray serverName)
{
    emit initWSS(ip, port, serverName);
}

bool Unik::startWSS(const QByteArray ip, const int port, const QByteArray serverName)
{
...
#else*/
bool Unik::startWSS(QByteArray ip, int port, QByteArray serverName)
{
...
bool Unik::sqliteInit(QString pathName)
{
    bool ret=false;
    if(db.isOpen()){
        db.close();
...
bool Unik::sqlQuery(QString query)
{
    QSqlQuery q;
    q.prepare(query);
    if(q.exec()){
...
QList<QObject *> Unik::getSqlData(QString query)
{
    QList<QObject *> ret;
    QSqlQuery consultar;
...
bool Unik::mysqlInit(QString hostName, QString dataBaseName, QString userName, QString password, int firtOrSecondDB)
{
    bool ret=false;
    if(firtOrSecondDB==1){
...
void Unik::setMySqlDatabase(QString databaseName, int firtOrSecondDB)
{
    if(firtOrSecondDB==1){
        firstDB.setDatabaseName(databaseName);
    }else{
...
QString Unik::currentFolderPath()
{
    QDir f(QDir::currentPath());
    return f.currentPath();
}

QString Unik::currentFolderName()
{
    QDir f(QDir::currentPath());
    return f.dirName();
}
void Unik::deleteFile(QByteArray f)
{
    QFile arch(f);
    arch.remove();
}

bool Unik::setFile(QByteArray fileName, QByteArray fileData)
{
    return setFile(fileName, fileData, "UTF-8");
}

bool Unik::setFile(QByteArray fileName, QByteArray fileData, QByteArray codec)
{
    QFile file(fileName);
...
QString Unik::getFile(QByteArray n)
{
    QString r;
    QFile file(n);
...
QList<QString> Unik::getFolderFileList(const QByteArray folder)
{
    QList<QString> ret;
    QDir d(folder);
...
bool Unik::fileExist(QByteArray fileName)
{
    QFile a(fileName);
    return a.exists();
}

QByteArray Unik::base64ToByteArray(const QByteArray data)
{
    return QByteArray::fromBase64(data);
}

QByteArray Unik::byteArrayToBase64(const QByteArray data)
{
    return data.toBase64();
}

QByteArray Unik::uCompressed(const QByteArray data)
{
...
#ifdef Q_OS_WIN
bool Unik::createLink(QString execString, QString arguments, QString lnkLocationFileName, QString description, QString workingDirectory)
{
...
#endif
#ifdef Q_OS_LINUX
bool Unik::createLink(QString execString, QString desktopLocationFileName, QString name, QString comment)
{
    return createLink(execString, desktopLocationFileName, name, comment, "");
}

bool Unik::createLink(QString execString, QString desktopLocationFileName, QString name, QString comment, QString iconPath)
{
...
QImage Unik::getScreen(int screen)
{
    QScreen *s = QGuiApplication::primaryScreen();
    return s->grabWindow(screen).toImage();
}

QByteArray Unik::imageCameraCapturaToByteArray(QString url)
{
    QImage qi(url);
    QByteArray ba;
    QBuffer bu(&ba);
...
QByteArray Unik::itemToImageData(QObject *item)
{
...
QByteArray Unik::screenImageData(int screen)
{
    QScreen *s = QGuiApplication::primaryScreen();
    QByteArray ba;
    QBuffer bu(&ba);
...
bool Unik::imageComparation(const QImage &firstImage, const QImage &secondImage, double lim)
{
...
Q_INVOKABLE QByteArray  Unik::sendAudioStreamWSS(const QString audioFilePath,  int bytes)
{
...
void Unik::appendAudioStreamFileWSS(const QString audioFilePath, const QByteArray data)
{
    QFile file(audioFilePath);
...
QString Unik::toHtmlEscaped(QString htmlCode)
{
    return htmlCode.toHtmlEscaped();
}

void Unik::crearPDF(QString captura, QString url, int orientacion)
{
...
Q_INVOKABLE void Unik::initRpiGpio()
{
#ifdef UNIK_COMPILE_RPI
    rpiGpio = new mmapGpio();
#endif
...
Q_INVOKABLE void Unik::setPinType(int pin, int type)
{
#ifdef UNIK_COMPILE_RPI
    if(type!=0&&type!=1){
        return;
...
Q_INVOKABLE void Unik::setPinState(int pin, int state)
{
#ifdef UNIK_COMPILE_RPI
    if(state!=0&&state!=1){
        return;
...
Q_INVOKABLE unsigned int Unik::readPin(unsigned int pin)
{
#ifdef UNIK_COMPILE_RPI
...
Q_INVOKABLE void Unik::writePinHigh(unsigned int pinnum)
{
#ifdef UNIK_COMPILE_RPI
...
Q_INVOKABLE void Unik::writePinLow(unsigned int pinnum)
{
#ifdef UNIK_COMPILE_RPI
    rpiGpio->writePinLow(pinnum);
#endif
}
Q_INVOKABLE bool Unik::pinIsHigh(int pin){
#ifdef UNIK_COMPILE_RPI
    unsigned int pinVal;
...
QString Unik::encPrivateData(QByteArray d, QString user, QString key)
{
    QString ret;
    QByteArray upkData;
    QByteArray r="6226";
...
QString Unik::decPrivateData(QByteArray d0, QString user, QString key)
{
    QString ret;
...
QString Unik::compData(QString d)
{
    QString nd=d;
...
QString Unik::desCompData(QString d)
{
    QString nd=d;
    for (int i = 0; i < lsim.size(); ++i) {
        QByteArray rs;
...
void Unik::downloadZipProgress(qint64 bytesSend, qint64 bytesTotal)
{    
    double porc;
    if(bytesTotal!=-1){
...
#ifdef Q_OS_WIN
int Unik::frameHeight(QObject *window)
{
...
int Unik::frameWidth(QObject *window)
{
    QQuickWindow *qw = qobject_cast<QQuickWindow *>(window);
    if(qw)
